...
if ($_POST[$APIKEY_FROM] != "" && $_POST[$APIKEY_TO] != "")
	...
	//Let's see if this is a "BitChirp API" "Chirp"
	if ($theto1 == "BM-2D853G9gtXwNajcbsNZXpr4eBm5YpcETWL")
		{
		//BitChirp API stuff. Start with "Voting" system	
		//Let's see if this is a new "BitChirp API" "Chirp"
		$query1 = "SELECT * FROM api_00 WHERE time = FROM_UNIXTIME(".$thetime1.") AND trans_category = '".$thesubject1."'";
		//run query
		$results1 = mysql_query($query1);
		if (mysql_num_rows($results1) == 0) // should be ZERO the post was not there before
			{
			//Store Vote000 transaction
			$query2 = "INSERT INTO api_00(time, trans_category, trans_string) VALUES(FROM_UNIXTIME(".$thetime1."),'".$thesubject1."','".$themessage1."')";
			}
		mysql_query($query2);
		}
	//Let's see if this is a new "Chirp"
	else
		{
